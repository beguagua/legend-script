...
struct Runtime { int next_entity=1;std::vector<Entity> entities;std::vector<Wait> coroutines; };
...
int main(int argc,char**argv){using namespace legend;std::unordered_map<std::string,Value> env;Runtime rt;try{if(argc>1&&std::string(argv[1])=="--version"){std::cout<<"Legend Script v0.2.0\n";return 0;}if(argc>1&&std::string(argv[1])=="--help"){std::cout<<"Usage:\n  legend file.lgnd\n  legend -e '<script>'\n  legend --compile file.lgnd [out.lbc]\n  legend --bytecode file.lbc\n  legend --watch file.lgnd\n";return 0;}if(argc>1&&std::string(argv[1])=="-e"){execute(argv[2],env,rt);return 0;}if(argc>1&&std::string(argv[1])=="--compile"){fs::path in=argv[2],out=argc>3?argv[3]:in.replace_extension(".lbc");write_bytecode(out,read_file(in));std::cout<<"compiled "<<in<<" -> "<<out<<'\n';return 0;}if(argc>1&&std::string(argv[1])=="--bytecode"){execute(read_bytecode(argv[2]),env,rt);return 0;}if(argc>1&&std::string(argv[1])=="--watch"){fs::path file=argv[2];auto stamp=fs::last_write_time(file);std::cout<<"hot reload watching "<<file<<"\n";for(;;){auto now=fs::last_write_time(file);if(now!=stamp){stamp=now;env.clear();rt=Runtime{};try{execute(read_file(file),env,rt);std::cout<<"reloaded\n";}catch(const std::exception&e){std::cerr<<"Legend error: "<<e.what()<<'\n';}}std::this_thread::sleep_for(std::chrono::milliseconds(200));} }if(argc>1){fs::path p=argv[1];if(p.extension()!=".lgnd")throw std::runtime_error("Legend files must use .lgnd");execute(read_file(p),env,rt);return 0;}std::cout<<"Legend Script v0.2.0\nlegend> ";std::string line;while(std::getline(std::cin,line)&&line!="exit"){execute(line,env,rt);std::cout<<"legend> ";}}catch(const std::exception&e){std::cerr<<"Legend error: "<<e.what()<<'\n';return 1;}}
